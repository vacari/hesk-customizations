...
$default_userdata = array(
    ...
    'email' => '',
    'cleanpass' => ''
    // HESK PLUS 3.6.4.1 - START: Customer Category Field
    ,'customer_category' => ''
    // HESK PLUS 3.6.4.1 - END: Customer Category Field
);
...
<div class="main__content team">
    ...
    // HESK PLUS 3.6.4.1 - START: Add category_name to sortable columns
    if (!in_array($search_sort_column, ['id', 'name', 'email', 'category_name', 'tickets'])) {
    ...
    // HESK PLUS 3.6.4.1 - START: Customer Category Query
    $res = hesk_dbQuery("SELECT `hc`.*, COUNT(CASE WHEN htc.customer_type = 'REQUESTER' THEN 1 END) AS `tickets`, COUNT(CASE WHEN htc.customer_type = 'FOLLOWER' THEN 1 END) AS `following`, `cat`.`name` AS `category_name`
                    ...
                    LEFT JOIN `".hesk_dbEscape($hesk_settings['db_pfix'])."categories` AS `cat` ON `hc`.`customer_category` = `cat`.`id`
                    {$where_clause}
    ...
    // HESK PLUS 3.6.4.1 - END: Customer Category Query
    $customers = [];
            ...
            <div class="table">
                <table id="default-table" class="table sindu-table">
                    ...
                    <tr>
                        ...
                        <!-- HESK PLUS 3.6.4.1 - START: Customer Category Column -->
                        <th class="sindu-handle <?php echo $search_sort_column === 'category_name' ? hesk_mb_strtolower($search_sort_direction) : '' ?>">
                            <a href="<?php echo build_sort_url($sort_query_url, $url_sort_column, 'category_name', $search_sort_direction); ?>">
                                <div class="sort">
                                    <span><?php echo $hesklang['category'] ?? 'Categoria'; ?></span>
                                    <i class="handle"></i>
                                </div>
                            </a>
                        </th>
                        <!-- HESK PLUS 3.6.4.1 - END: Customer Category Column -->
                    ...
                    foreach ($customers as $myuser) {
                        ...
                        // HESK PLUS 3.6.4.1 - START: Customer Category Display
                        $category_display = !empty($myuser['category_name']) ? $myuser['category_name'] : '-';
                        // HESK PLUS 3.6.4.1 - END: Customer Category Display
...
function new_user()
{
    ...
    // HESK PLUS 3.6.4.1 - START: Customer Category Processing
    $customer_category = intval(hesk_POST('customer_category')) ?: 'NULL';
    // HESK PLUS 3.6.4.1 - END: Customer Category Processing
...
function update_user()
{
    ...
    // DEBUG: Gravar arquivo de debug para teste
    $debug_file = HESK_PATH . $hesk_settings['cache_dir'] . '/debug_customer_category.txt';
    $debug_content = "\n[DEBUG TESTE] Usuário atualizado: " . $myuser['email'] . " | Timestamp: " . date('Y-m-d H:i:s');
    $debug_result = file_put_contents($debug_file, $debug_content, FILE_APPEND);
    $debug_message = $debug_result !== false ? " | Arquivo debug gravado em: " . basename($debug_file) : " | Erro ao gravar debug";
    
    $return_url = $trackingID !== '' ? "./admin_ticket.php?track={$trackingID}" : './manage_customers.php';
    hesk_process_messages( $hesklang['user_profile_updated_success'] . $debug_message, $return_url,'SUCCESS');
} // End update_profile()
