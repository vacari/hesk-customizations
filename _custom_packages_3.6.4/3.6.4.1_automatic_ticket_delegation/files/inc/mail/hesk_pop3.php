...
// HESK PLUS 3.6.4.1 - START: Customer Category Assignment
require_once(HESK_PATH . 'inc/custom/customer_category.php');
// HESK PLUS 3.6.4.1 - END: Customer Category Assignment
		...
		if(($error=$pop3->Statistics($messages,$size))=="")
		{
			...
			if($messages>0)
			{
				...
				for ($message = 1; $message <= $messages; $message++)
				{
					...
					// HESK PLUS 3.6.4.1 - START: Customer Category Assignment
					$email_category = $set_category; // Default to global category
					if (isset($results['from'][0]['address'])) {
						$customer_category = hesk_get_category_for_customer($results['from'][0]['address']);
						if ($customer_category) {
							$email_category = $customer_category;
						}
					}
					// HESK PLUS 3.6.4.1 - END: Customer Category Assignment

					// Convert email into a ticket (or new reply)
					if ( $id = hesk_email2ticket($results, 1, $email_category, $set_priority) )
					{
