...
function hesk_customer_tab($session_array='new', $customer_verified = false)
{
    ...
    <div class="step-slider">
        <div class="step-item step-1">
            ...
            <div class="form-group">
                <label for="prof_customer_category"><?php echo $hesklang['category'] ?? 'Categoria'; ?></label>
                <div class="dropdown-select out-close">
                    <select class="form-control <?php echo in_array('customer_category', $errors) ? 'isError' : ''; ?>" id="prof_customer_category" name="customer_category">
                        <option value=""><?php echo $hesklang['select_category'] ?? 'Selecionar categoria...'; ?></option>
                        <?php
                        // Buscar categorias disponíveis
                        $res = hesk_dbQuery("SELECT `id`, `name` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."categories` ORDER BY `name`");
                        while ($row = hesk_dbFetchAssoc($res)) {
                            $selected = (isset($_SESSION[$session_array]['customer_category']) && $_SESSION[$session_array]['customer_category'] == $row['id']) ? 'selected="selected"' : '';
